...
export default function PickBanner({ rows, rules }: { rows: AdRow[]; rules: Rules }) {
  ...
  if (!best) {
    return (
      <div className="flex items-center gap-3.5 bg-gradient-to-br from-accentDim to-surface border border-accent/25 rounded-card px-5 py-4">
        <Icon />
        <div>
          <div className="text-[12px] font-bold uppercase tracking-wide text-accent">ยังสรุปไม่ได้</div>
  ...
  return (
    <div className="flex items-center gap-3.5 bg-gradient-to-br from-accentDim to-surface border border-accent/25 rounded-card px-5 py-4">
      <Icon />
      <div>
        <div className="text-[12px] font-bold uppercase tracking-wide text-accent">เส้นทางที่แนะนำ · อิมเพรสชันต่อบาทสูงสุด</div>
        <div className="font-display text-[16px]">
          {best.row.adsetName || "(ไม่มีชื่อ)"} — {fmt(best.v.impPerBaht, 2)} อิมเพรสชัน/฿
          {!best.hasHealthy && <span className="text-bad font-bold"> (ทุกแถวยังเข้าเกณฑ์ &quot;ควรปิด&quot; — พิจารณาปรับก่อนเปิดต่อ)</span>}
        </div>
        <div className="mt-1">
...
function Icon() {
  return (
    <div className="w-10 h-10 rounded-[10px] bg-accent text-accentInk flex items-center justify-center flex-none">
